...
#ifndef NPPDX_EXAMPLE_COMMON_NVRTC_HPP
#define NPPDX_EXAMPLE_COMMON_NVRTC_HPP

#include <cuda_fp16.h>
#include <cstdlib>
#include <sstream>
#define NPPDX_EXAMPLE_NVRTC
...
namespace example {
    namespace nvrtc {
        ...
        inline std::vector<std::string> get_npp_include_dirs() {

            std::vector<std::string> npp_include_dirs_array;

            const auto path_handler = [&](const std::string& path_name, const std::string& entry) {
                if (!entry.empty()) {
                    npp_include_dirs_array.push_back("--include-path=" + entry);
                } else {
            ...
            {
                const char* env_ptr = std::getenv("NPPDX_EXAMPLE_COMMONDX_INCLUDE_DIR");
                if (env_ptr != nullptr) {
                    path_handler("NPPDX_EXAMPLE_COMMONDX_INCLUDE_DIR", std::string(env_ptr));
                } else {
            ...
            {
                const char* env_ptr = std::getenv("NPPDX_EXAMPLE_NPPDX_INCLUDE_DIR");
                if (env_ptr != nullptr) {
                    path_handler("NPPDX_EXAMPLE_NPPDX_INCLUDE_DIR", std::string(env_ptr));
                } else {
#ifdef NPPDX_INCLUDE_DIRS
                    { append_multiple_dirs(npp_include_dirs_array, std::string(NPPDX_INCLUDE_DIRS)); }
#endif
                }
            }
            {
                const char* env_ptr = std::getenv("NPPDX_EXAMPLE_CUTLASS_INCLUDE_DIR");
                if (env_ptr != nullptr) {
                    path_handler("NPPDX_EXAMPLE_CUTLASS_INCLUDE_DIR", std::string(env_ptr));
                } else {
#ifdef CUTLASS_INCLUDE_DIR
                    { path_handler("CUTLASS_INCLUDE_DIR", std::string(CUTLASS_INCLUDE_DIR)); }
#endif
                }
            }
            {
                const char* env_ptr = std::getenv("NPPDX_EXAMPLE_CUDA_INCLUDE_DIR");
                if (env_ptr != nullptr) {
                    path_handler("NPPDX_EXAMPLE_CUDA_INCLUDE_DIR", std::string(env_ptr));
// CUDA 13 created a separate include folder for CCCL
#if CUDA_VERSION >= 13000
                    path_handler("NPPDX_EXAMPLE_CUDA_INCLUDE_DIR_CCCL", std::string(env_ptr) + "/cccl");
#endif
                } else {
...
#    endif
            ...
            {
                const char* env_ptr = std::getenv("NPPDX_EXAMPLE_USER_DIRECTORIES");
                if (env_ptr != nullptr) {
                    append_multiple_dirs(npp_include_dirs_array, std::string(env_ptr));
                }
            }
            return npp_include_dirs_array;
        }
...
#endif // NPPDX_EXAMPLE_COMMON_NVRTC_HPP
